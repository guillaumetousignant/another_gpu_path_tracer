...
namespace AGPTracer::Entities {
     ...
     */
    export template<template<typename> typename S, typename T>
     ...
     */
    export template<template<typename> typename S, typename T, typename T2>
    ...
    concept Transformable = requires(const S<T> a) {
        { a.transformation_ } -> std::convertible_to<TransformationMatrix_t<T>>;
    };
